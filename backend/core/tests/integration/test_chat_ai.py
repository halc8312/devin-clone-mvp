...
class TestChatAPI:
    ...
    async def test_list_chat_sessions(
        ...
        response = await client.get(
            f"/api/v1/projects/{test_project.id}/chat/sessions", headers=auth_headers
        )
    ...
    async def test_stream_chat_response(
        self, client, auth_headers, test_project, db_session, mock_anthropic_client
    ):
...
class TestCodeGeneration:
    ...
    async def test_generate_code(
        self, client, auth_headers, test_project, mock_anthropic_client
    ):
...
class TestChatWithFileContext:
    ...
    async def test_chat_with_file_references(
        self,
        client,
        auth_headers,
        test_project,
