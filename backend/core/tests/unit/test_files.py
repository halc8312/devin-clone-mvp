...
class TestFileAPI:
    ...
    async def test_list_files(self, client, auth_headers, test_project, test_file):
        ...
        response = await client.get(
            f"/api/v1/projects/{test_project.id}/files", headers=auth_headers
        )
    ...
    async def test_move_file(
        self, client, auth_headers, test_project, test_file, db_session
    ):
        """Test moving a file."""
    ...
    async def test_file_tree(self, client, auth_headers, test_project, db_session):
        ...
        response = await client.get(
            f"/api/v1/projects/{test_project.id}/files/tree", headers=auth_headers
        )
