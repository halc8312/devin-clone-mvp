...
async def get_claude_model(db: AsyncSession, model_id: str = None) -> str:
        ...
        result = await db.execute(
            select(ClaudeModel).where(
                and_(
                    ...
                    ClaudeModel.is_active is True,
                    ClaudeModel.is_deprecated is False,
                )
            )
        )
    ...
    result = await db.execute(
        select(ClaudeModel).where(
            and_(
                ClaudeModel.is_default is True,
                ClaudeModel.is_active is True,
                ClaudeModel.is_deprecated is False,
            )
        )
    ...
    *,
    ...
    result = await db.execute(
        select(ChatSession).where(
            and_(ChatSession.id == session_id, ChatSession.project_id == project_id)
        )
    )
    ...
    return ChatSessionWithMessages(**session.__dict__, messages=messages)
...
Current project context: {project.name} - {project.description or 'No description'}"""
    ...
    try:
        response = await claude_client.create_message(
            messages=messages, system=system_prompt, max_tokens=4096, model=model_to_use
        )
    ...
    async def generate():
        ...
        try:
            async for chunk in claude_client.create_message_stream(
                messages=messages, system=system_prompt, model=model_to_use
            ):
                full_response += chunk
...
@router.post(
    "/projects/{project_id}/code/generate", response_model=CodeGenerationResponse
)
    ...
    *,
    ...
    try:
        explanation = await code_assistant.explain_code(
            code=request.code, language=request.language or project.language
        )
