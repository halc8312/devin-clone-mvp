...
async def list_available_prices(
    *,
    ...
    result = await db.execute(
        select(PriceProduct)
        .where(PriceProduct.active is True)
...
async def create_checkout_session(
    *,
    ...
    if not price:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Invalid price ID"
        )
    ...
    try:
        success_url = (
            checkout_request.success_url
            or f"{settings.FRONTEND_URL}/settings/billing?success=true"
        )
        cancel_url = (
            checkout_request.cancel_url or f"{settings.FRONTEND_URL}/settings/billing"
        )
...
async def create_portal_session(
    *,
    ...
    if not current_user.stripe_customer_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No customer account found"
        )

    try:
        return_url = (
            portal_request.return_url or f"{settings.FRONTEND_URL}/settings/billing"
        )
...
async def list_payments(
    *,
    ...
    return PaymentList(payments=payments, total=total, page=page, page_size=page_size)
