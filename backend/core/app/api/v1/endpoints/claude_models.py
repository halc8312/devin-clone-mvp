...
async def get_default_model(db: AsyncSession = Depends(get_db)):
    ...
    if not model:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No default model configured"
        )
    return model
...
async def create_claude_model(
    model: ClaudeModelCreate, db: AsyncSession = Depends(get_db)
):
...
async def update_claude_model(
    model_id: str, model_update: ClaudeModelUpdate, db: AsyncSession = Depends(get_db)
):
...
async def set_default_model(
    request: ModelSelectionRequest, db: AsyncSession = Depends(get_db)
):
    """Set a model as the default"""
    model = await crud.set_default_model(db=db, model_id=request.model_id)
    if not model:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Model not found or inactive"
        )
...
async def initialize_default_models(db: AsyncSession = Depends(get_db)):
    ...
    for model_data in default_models:
        ...
        if existing_model:
            ...
            update_data = ClaudeModelUpdate(
                **{k: v for k, v in model_data.items() if k != "model_id"}
            )
            await crud.update_model(
                db=db, model_id=model_data["model_id"], model_update=update_data
            )
            updated_count += 1
        else:
            # Create new model
