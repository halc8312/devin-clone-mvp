...
async def list_project_files(
    *,
    ...
    sum(1 for f in files if f.type == FileType.FILE)
...
async def get_file_tree(
    *,
    ...
    def build_tree(node: ProjectFile) -> ProjectFileTree:
        children = [build_tree(child) for child in files if child.parent_id == node.id]

        tree_node = ProjectFileTree(**node.__dict__)
        tree_node.children = sorted(
            children, key=lambda x: (x.type != FileType.DIRECTORY, x.name.lower())
        )
        return tree_node
    ...
    *,
    ...
    existing_result = await db.execute(
        select(ProjectFile).where(
            and_(ProjectFile.project_id == project_id, ProjectFile.path == file_in.path)
        )
    )
    ...
    db_file = ProjectFile(
        **file_in.dict(),
        ...
        language=get_file_language(file_in.name)
        if file_in.type == FileType.FILE
        else None,
        size_bytes=len(file_in.content.encode()) if file_in.content else 0,
        mime_type=mimetypes.guess_type(file_in.name)[0]
        if file_in.type == FileType.FILE
        else None,
    )
...
async def get_file(
    *,
    ...
    result = await db.execute(
        select(ProjectFile).where(
            and_(ProjectFile.id == file_id, ProjectFile.project_id == project_id)
        )
    )
