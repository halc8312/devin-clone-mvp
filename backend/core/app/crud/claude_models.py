...
async def get_model(db: AsyncSession, model_id: str) -> Optional[ClaudeModel]:
    ...
    result = await db.execute(
        select(ClaudeModel).where(ClaudeModel.model_id == model_id)
    )
...
async def get_models(
    ...
    if active_only:
        query = query.where(ClaudeModel.is_active is True)

    if not include_deprecated:
        query = query.where(ClaudeModel.is_deprecated is False)

    query = (
        query.order_by(ClaudeModel.model_family, ClaudeModel.model_tier)
        .offset(skip)
        .limit(limit)
    )
...
async def get_default_model(db: AsyncSession) -> Optional[ClaudeModel]:
    ...
    result = await db.execute(
        select(ClaudeModel).where(
            and_(ClaudeModel.is_default is True, ClaudeModel.is_active is True)
        )
    )
    return result.scalar_one_or_none()
...
async def create_model(db: AsyncSession, model: ClaudeModelCreate) -> ClaudeModel:
    ...
    if model.is_default:
        await db.execute(select(ClaudeModel).where(ClaudeModel.is_default is True))
        # Update all existing defaults to False
        existing_defaults = await db.execute(
            select(ClaudeModel).where(ClaudeModel.is_default is True)
        )
...
async def update_model(
    db: AsyncSession, model_id: str, model_update: ClaudeModelUpdate
